...
public class TripUploader extends AsyncTask<Long, Integer, Boolean> {
	...
	private JSONObject getCoordsJSON(long tripId) throws JSONException {
		...
		fieldMap.put(TRIP_COORDS_TIME,
				tripCoordsCursor.getColumnIndex(edu.utk.cycleushare.cycleknoxville.DbAdapter.K_POINT_TIME));
		fieldMap.put(TRIP_COORDS_LAT,
				tripCoordsCursor.getColumnIndex(edu.utk.cycleushare.cycleknoxville.DbAdapter.K_POINT_LAT));
		fieldMap.put(TRIP_COORDS_LON,
				tripCoordsCursor.getColumnIndex(edu.utk.cycleushare.cycleknoxville.DbAdapter.K_POINT_LGT));
		fieldMap.put(TRIP_COORDS_ALT,
				tripCoordsCursor.getColumnIndex(edu.utk.cycleushare.cycleknoxville.DbAdapter.K_POINT_ALT));
		fieldMap.put(TRIP_COORDS_SPEED,
				tripCoordsCursor.getColumnIndex(edu.utk.cycleushare.cycleknoxville.DbAdapter.K_POINT_SPEED));
		fieldMap.put(TRIP_COORDS_HACCURACY,
				tripCoordsCursor.getColumnIndex(edu.utk.cycleushare.cycleknoxville.DbAdapter.K_POINT_ACC));
	...
	private JSONObject getUserJSON() throws JSONException {
		...
		user.put(USER_AGE, settings.getInt("" + edu.utk.cycleushare.cycleknoxville.FragmentUserInfo.PREF_AGE, 0));
		user.put(USER_GENDER,
				settings.getInt("" + edu.utk.cycleushare.cycleknoxville.FragmentUserInfo.PREF_GENDER, 0));
		user.put(USER_CYCLING_FREQUENCY,
				settings.getInt("" + edu.utk.cycleushare.cycleknoxville.FragmentUserInfo.PREF_GENDER, 0) / 100);
		// Integer.parseInt(settings.getString(""+UserInfoActivity.PREF_CYCLEFREQ,
		// "0"))
		user.put(USER_ETHNICITY,
				settings.getInt("" + edu.utk.cycleushare.cycleknoxville.FragmentUserInfo.PREF_ETHNICITY, 0));
		user.put(USER_INCOME,
				settings.getInt("" + edu.utk.cycleushare.cycleknoxville.FragmentUserInfo.PREF_INCOME, 0));
		user.put(USER_RIDERTYPE,
				settings.getInt("" + edu.utk.cycleushare.cycleknoxville.FragmentUserInfo.PREF_RIDERTYPE, 0));
		user.put(USER_RIDERHISTORY,
				settings.getInt("" + edu.utk.cycleushare.cycleknoxville.FragmentUserInfo.PREF_RIDERHISTORY, 0));
	...
	private Vector<String> getTripData(long tripId) {
		...
		String note = tripCursor.getString(tripCursor
				.getColumnIndex(edu.utk.cycleushare.cycleknoxville.DbAdapter.K_TRIP_NOTE));
		String purpose = tripCursor.getString(tripCursor
				.getColumnIndex(edu.utk.cycleushare.cycleknoxville.DbAdapter.K_TRIP_PURP));
		Double startTime = tripCursor.getDouble(tripCursor
				.getColumnIndex(edu.utk.cycleushare.cycleknoxville.DbAdapter.K_TRIP_START));
		Double endTime = tripCursor.getDouble(tripCursor
				.getColumnIndex(edu.utk.cycleushare.cycleknoxville.DbAdapter.K_TRIP_END));
		tripCursor.close();
		mDb.close();
	...
	boolean uploadOneTrip(long currentTripId) {
		...
		// TODO: Server URL
		final String postUrl = "http://cycleknox.phillipgoldfarb.com/submit_trip.php";
		...
		try {
			...
			if (responseData.getString("status").equals("success")) {
				mDb.open();
				mDb.updateTripStatus(currentTripId, edu.utk.cycleushare.cycleknoxville.TripData.STATUS_SENT);
				mDb.close();
				result = true;
			}
	...
	private edu.utk.cycleushare.cycleknoxville.SavedTripsAdapter mSavedTripsAdapter;

	public edu.utk.cycleushare.cycleknoxville.SavedTripsAdapter setSavedTripsAdapter(
			edu.utk.cycleushare.cycleknoxville.SavedTripsAdapter mSavedTripsAdapter) {
		this.mSavedTripsAdapter = mSavedTripsAdapter;
		return mSavedTripsAdapter;
	}

	private edu.utk.cycleushare.cycleknoxville.FragmentSavedTripsSection fragmentSavedTripsSection;

	public edu.utk.cycleushare.cycleknoxville.FragmentSavedTripsSection setFragmentSavedTripsSection(
			edu.utk.cycleushare.cycleknoxville.FragmentSavedTripsSection fragmentSavedTripsSection) {
		...
		try {
			...
			if (result) {
				...
				Toast.makeText(
						...
						"I BIKE KNX couldn't upload the trip, and will retry when your next trip is completed.",
						Toast.LENGTH_LONG).show();
			}
		} catch (Exception e) {
